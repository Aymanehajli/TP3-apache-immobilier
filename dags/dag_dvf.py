...
def pipeline_dvf():
    ...
    def traiter_donnees(hdfs_path: str) -> dict:
        ...
        now = datetime.now()
        grouped["annee"] = now.year
        grouped["mois"] = now.month
    ...
    def inserer_postgresql(resultats: dict) -> int:
        ...
        for row in agregats:
            params = (
                ...
                int(row.get("arrondissement")),
                int(row.get("annee")),
                int(row.get("mois")),
                float(row.get("prix_m2_moyen")) if row.get("prix_m2_moyen") is not None else None,
                float(row.get("prix_m2_median")) if row.get("prix_m2_median") is not None else None,
                float(row.get("prix_m2_min")) if row.get("prix_m2_min") is not None else None,
                float(row.get("prix_m2_max")) if row.get("prix_m2_max") is not None else None,
                int(row.get("nb_transactions")) if row.get("nb_transactions") is not None else 0,
                float(row.get("surface_moyenne")) if row.get("surface_moyenne") is not None else None,
            )
        ...
        if stats_globales:
            params_stats = (
                int(stats_globales.get("annee")),
                int(stats_globales.get("mois")),
                int(stats_globales.get("nb_transactions_total") or 0),
                float(stats_globales.get("prix_m2_median_paris"))
                if stats_globales.get("prix_m2_median_paris") is not None
                else None,
                float(stats_globales.get("prix_m2_moyen_paris"))
                if stats_globales.get("prix_m2_moyen_paris") is not None
                else None,
                int(stats_globales.get("arrdt_plus_cher"))
                if stats_globales.get("arrdt_plus_cher") is not None
                else None,
                int(stats_globales.get("arrdt_moins_cher"))
                if stats_globales.get("arrdt_moins_cher") is not None
                else None,
                float(stats_globales.get("surface_mediane"))
                if stats_globales.get("surface_mediane") is not None
                else None,
            )
